<x-ui.page title="Notifications" description="In-app alerts for scheduled tasks, budget spending, and media distribution.">
        ...
        <div class="flex items-center gap-2">
            ...
            @if(auth()->user()->notifications->count() > 0)
                <form method="POST" action="{{ route('notifications.clear-all') }}">
                    @csrf @method('DELETE')
                    <x-ui.button type="submit" variant="danger" size="sm" onclick="return confirm('Clear all notifications permanently?')">Clear all</x-ui.button>
                </form>
            @endif
        </div>
    </x-slot>
    ...
    <div class="space-y-4">
        @forelse($notifications as $n)
            ...
            <div @class([
                'ui-card flex items-start gap-4 transition-all !py-4.5 border-l-4 relative',
                ...
                <div class="flex-1 min-w-0 pr-12">
                    <div class="flex items-center gap-2">
                        <span class="text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)]">
                ...
                <div class="absolute right-4 top-4 flex items-center gap-1.5">
                    ...
                    <form method="POST" action="{{ route('notifications.destroy', $n->id) }}">
                        @csrf @method('DELETE')
                        <button type="submit" class="p-1.5 rounded-lg text-rose-500 hover:bg-rose-500/10 transition-colors" title="Delete notification" onclick="return confirm('Delete notification?')">
